...
_ai_ = r'\adjustimage{{max size={{{0}\textwidth}}{{{0}\paperheight}}}}{{{1}}}'
    ...
    def toHTML(self, table):
        ...
        if self.table.caption is not None:
            if self.__numbering_tables__:
                html.append('<caption>Table {}: {}</caption>'.format(
                    self.__table_n__, self.table.caption))
            else:
        ...
        while(n_iter < len(self.table.images)):
            ...
            for i in range(self.table.n_cols):
                html.append('<td><img src={} ></img></td>'.format(
                    self.table.images[n_iter]))
                n_iter += 1
        ...
        if not self.host.n_cols == 0:
            ...
            for image in self.host.images:
                if current_col == self.host.n_cols-1:
                    out += r'{} \\ \hline '.format(_ai_.format(
                        1.0/self.host.n_cols, image))
                    current_col = 0
                else:
                    out += r'{} & '.format(_ai_.format(
                        1.0/self.host.n_cols, image))
                    current_col += 1
            out += r'\end{tabular}'
